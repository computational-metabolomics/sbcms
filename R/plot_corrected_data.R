...
##'
##' @param df A data frame containing the original data before correction (samples in columns, features in rows).
##' @param corrected_df A data frame containing the corrected data, as output by QCRSC
...
plot_sbc <- function (df, corrected_df, classes, batch, order, output="sbcms_plots.pdf")
{
  ...
  gg_THEME <- theme(
    ...
    panel.grid.minor.x=element_line(color="gray80", size=0.3, linetype="dashed"),
  ...
  for (peakn in 1:plotLim){
    cat (peakn, "\n")
    A <- data.frame (x=c(1:ncol(df)), original=log(df[peakn, ],10), fitted=log(corrected_df[peakn, ],10),
