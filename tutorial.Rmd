---
title: "Tutorial: Signal and Batch Correction for Mass Spectrometry (SBCMS)"
...
We will be using a small subset of 20 features of an DIMS data set consisting of 172 samples measured across 8 batches.
More detailed description of the data set is available from [Kirwan et al, Scientific Data, 1, 140012 (2014)](https://www.nature.com/articles/sdata201412) and [MTBLS79] (https://www.ebi.ac.uk/metabolights/MTBLS79)
...
plotdata$feature <- factor(plotdata$feature, ordered=TRUE, levels=unique(plotdata$feature))
...
plotdata2$index <- factor(plotdata2$index, ordered=TRUE, levels=unique(plotdata2$index))
...
pca_data <- pmp::mv_imputation(pca_data, method="KNN", k=5, rowmax=0.5, colmax=0.5, maxp=NULL, check_df=FALSE)
...
for (coln in 1:ncol(data)) {
  for (bch in 1:8) {
    sampleorder <- scale(c(1:length(which(QCbatch==bch))), center=TRUE, scale=TRUE)
...
Figure above indicates that there are significant acquisition order related trends within some batches. For example, bathes 4 and 6 for features '451.01081' and '409.05716'.
...
# Note that input for QCRSC function needs samples to be in columns and features in rows
data <- t(data)
...
pca_corrected_data <- pmp::mv_imputation(pca_corrected_data, method="KNN", k=5, rowmax=0.5, colmax=0.5, maxp=NULL, check_df=FALSE)
